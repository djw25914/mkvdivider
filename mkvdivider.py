...
# GPL v3
## Small python3 kde4 program to divide MKV files based on chapter information.
# Version 0.3.0
...
try:
    ...
    print("Your MKV Splitter installation is incomplete. Check that mydirrequester.py, timetools.py, qtruncommmand  are present.")
    sys.exit(2)
...
class MKVDivider(QWidget):
    ...
    def __init__(self):
        ...
        self.divide_button.setText('Split')
        ...
        self.chapters_button_box.addWidget(self.divide_button)
    ...
    def preview_chapter(self):
        ...
        run_command_basic("mplayer -chapter {0}-{1} {2}".format(current_row+1, current_row+1, self.input_file))
    ...
    def perform_division(self):
        ...
        for row in range(self.chapters_table.rowCount()):
            if self.chapters_table.cellWidget(row, self.SPLIT).isChecked():
                counter = counter + 1
                division_points.append(str(self.chapters_table.item(row, self.START_TIME).text()))
        ...
        if counter > 1: #There is something to split
            ...
            division_points = ','.join(division_points)
...
app_name = "mkv-divider"
catalog = ""
program_name = ki18n("mkv-divider")
version = "0.3.0"
